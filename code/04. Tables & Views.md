...
```
CREATE TABLE TABLE2 (
	TABLE2_ID SERIAL PRIMARY KEY, ---- SERIAL -> +1 on ID for each row 
	TABLE2_name VARCHAR(100) NOT NULL,
	TABLE2_length INT,
	TABLE2_lang VARCHAR(20),
	DAATEUPD DATE,
		TABLE_ID INT REFERENCES TABLE (TABLE_ID) -> FOREIGN KEY confirguration (requires prior configuration of TABLE)
);
```
